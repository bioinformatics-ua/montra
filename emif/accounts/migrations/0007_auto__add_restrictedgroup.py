...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'RestrictedGroup'
        db.create_table('accounts_restrictedgroup', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('group', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['auth.Group'], unique=True)),
        ))
        db.send_create_signal('accounts', ['RestrictedGroup'])

        # Adding M2M table for field fingerprints on 'RestrictedGroup'
        db.create_table('accounts_restrictedgroup_fingerprints', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('restrictedgroup', models.ForeignKey(orm['accounts.restrictedgroup'], null=False)),
            ('fingerprint', models.ForeignKey(orm['fingerprint.fingerprint'], null=False))
        ))
        db.create_unique('accounts_restrictedgroup_fingerprints', ['restrictedgroup_id', 'fingerprint_id'])


    def backwards(self, orm):
        # Deleting model 'RestrictedGroup'
        db.delete_table('accounts_restrictedgroup')

        # Removing M2M table for field fingerprints on 'RestrictedGroup'
        db.delete_table('accounts_restrictedgroup_fingerprints')
